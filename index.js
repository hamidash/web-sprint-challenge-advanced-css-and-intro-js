...
*/
function getArtistByIndex(id, artistObj) {
  artistObj = artistObj[id].name;
  return `The artist at index ${id} is ${artistObj}`;
    /* code here */
  }

  console.log(getArtistByIndex(5,artists));
...
*/
function removeArtist(artistObj,index) {
  artistObj = artistObj.splice(index,1);
  console.log(artistObj);
  console.log(artists.length);
  }
  removeArtist(artists,11);
  /**
...
const newArtist = {
  ...
  "genre": 'Web Dev', 
  "nationality": "Martian",
  "bio": 'Hamida is famous for her great cooking skills :) '
}
...
function lotsOfArt(artistObj){
  let manyPaintings = [];

  for(let i=0; i<artistObj.length; i++){
    if(artistObj[i].paintings > 100) {
        manyPaintings.push(artistObj[i].name);
      }
  }
